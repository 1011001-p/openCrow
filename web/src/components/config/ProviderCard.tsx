...
export function ProviderCard({
  ...
  const [modelsProbeResult, setModelsProbeResult] = useState<ProviderModelsProbeResult | null>(
    null,
  );
  ...
  return (
    <div className="rounded-xl border border-outline-ghost bg-surface-low overflow-hidden">
      ...
      >
        ...
        {displayStatus && (
          <span
            className={`text-xs font-mono shrink-0 ${displayStatus.ok ? "text-green-400" : "text-red-400"}`}
          >
        ...
        <div className="px-4 pb-4 space-y-4 border-t border-outline-ghost pt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Select
              ...
              onChange={(e) =>
                updateConfig((c) => {
                  c.llm.providers[i].kind = e.target.value;
                  return c;
                })
              }
            />
              ...
              <div className="space-y-2">
                {availableModels.length > 0 ? (
                  <Select
                    ...
                    onChange={(e) =>
                      updateConfig((c) => {
                        c.llm.providers[i].model = e.target.value;
                        return c;
                      })
                    }
                  />
                ...
                <div className="flex items-center gap-2">
                  ...
                  {modelsProbeResult && (
                    <span
                      className={`text-xs font-mono ${modelsProbeResult.ok ? "text-cyan" : "text-error"}`}
                    >
          ...
          {testResult && (
            <div
              className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm border ${testResult.ok ? "bg-green-400/10 text-green-400 border-green-400/20" : "bg-red-400/10 text-red-400 border-red-400/20"}`}
            >
              ...
              {testResult.error && (
                <span className="text-xs opacity-80 ml-1 truncate">{testResult.error}</span>
              )}
            </div>
          )}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  updateConfig((c) => {
                    c.llm.providers[i].priority = parseInt(e.target.value, 10) || 0;
                    return c;
                  })
                }
              />
