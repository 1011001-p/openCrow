...
export function MemoryRow({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-surface-mid overflow-hidden">
      ...
      >
        <span className="text-xs font-mono text-cyan/70 bg-cyan/10 px-1.5 py-0.5 rounded shrink-0">
          {mem.category || "--"}
        </span>
        ...
        <svg
          className={`w-3.5 h-3.5 text-on-surface-variant transition-transform flex-shrink-0 ${expanded ? "rotate-180" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Input
              ...
              onChange={(e) =>
                onUpdate({
                  ...mem,
                  confidence: Math.min(100, Math.max(0, parseInt(e.target.value) || 50)),
                })
              }
            />
          </div>
