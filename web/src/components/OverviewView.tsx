...
function WorkerTerminal({ workerKey, label }: { workerKey: string; label: string }) {
  ...
  useEffect(() => {
    let active = true;
    const fetchLogs = () => {
      endpoints.getWorkerLogs(workerKey).then((res) => { if (active) setEntries(res.entries ?? []); }).catch(() => {});
    };
    const fetchStats = () => {
      endpoints.getWorkerStatus().then((res) => {
        if (active) setStat(res.workers?.find((w) => w.name === workerKey) ?? null);
      }).catch(() => {});
    };
    fetchLogs();
    fetchStats();
    const id = setInterval(() => { fetchLogs(); fetchStats(); }, 3000);
  ...
  return (
    <div className="flex flex-col rounded-sm overflow-hidden" style={{ height: "260px", border: `1px solid ${TERMINAL_BORDER}` }}>
      {/* Title bar */}
      <div className="shrink-0 flex items-center gap-2 px-4 py-2" style={{ background: TERMINAL_HEADER_BG, borderBottom: `1px solid ${TERMINAL_BORDER}` }}>
        ...
        <span className="text-xs font-mono" style={{ color: TERMINAL_TEXT_DIM }}>{label}</span>
        {stat && stat.ticks > 0 && (
      ...
      {/* Log body */}
      <div className="flex-1 overflow-y-auto px-4 py-3 font-mono text-xs leading-relaxed" style={{ background: TERMINAL_BG }}>
        {entries.length === 0 ? (
          <p className="italic" style={{ color: TERMINAL_TEXT_DIM }}>No logs yet...</p>
        ) : (
          entries.map((e, i) => {
            const isError = e.line.toLowerCase().includes("error") || e.line.toLowerCase().includes("failed");
            const ts = new Date(e.ts).toLocaleTimeString();
            return (
              <div key={i} className="flex gap-2 leading-5">
                <span className="shrink-0 select-none" style={{ color: TERMINAL_TEXT_DIM }}>{ts}</span>
                <span style={{ color: isError ? "#f59e0b" : "rgba(236,242,255,0.75)" }}>{e.line}</span>
              </div>
            );
          })
        )}
...
function StatusCard({
  ...
  return (
    <div className={`rounded-sm border ${borderColor} ${bgColor} p-4`}>
      <p className="text-xs font-mono uppercase tracking-wider text-on-surface-variant mb-3">{title}</p>
      {children}
    </div>
  );
}
...
function DefaultChannelCard() {
  ...
  const [testResult, setTestResult] = useState<{ ok: boolean; latencyMs?: number; detail?: string; error?: string } | null>(null);
  const [testing, setTesting] = useState(false);

  useEffect(() => {
    endpoints.getConfig().then((cfg) => {
  ...
  const cardStatus: "ok" | "error" | "idle" =
    testing || bot === undefined ? "idle" :
    !bot ? "idle" :
    testResult ? (testResult.ok ? "ok" : "error") : "idle";
  ...
  return (
    <StatusCard title="Default Channel" status={cardStatus}>
      <div className="flex items-center gap-3">
        ...
        {bot === null && <span className="text-sm text-on-surface-variant">No Telegram channel configured</span>}
        {bot && testing && <span className="text-sm text-on-surface-variant font-mono">Checking {bot.label || "bot"}...</span>}
        {bot && !testing && testResult && (
          <span className="text-sm text-on-surface">
            {testResult.ok ? (
              <>
                <span className="font-mono text-cyan">{bot.label || "Telegram"}</span>
                {testResult.detail && <span className="text-on-surface-variant ml-2">{testResult.detail}</span>}
                {testResult.latencyMs != null && (
                  <span className="text-on-surface-variant ml-2 font-mono text-xs">{testResult.latencyMs}ms</span>
                )}
              </>
            ) : (
